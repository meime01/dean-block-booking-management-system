...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($requested_projectors <= $available_projectors && $requested_laptops <= $available_laptops) {
        ...
        $sql = "INSERT INTO bookings (booking_date, start_time, end_time, projectors, laptops, customer_id, department, phone_extension, phone_number)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssiissss", $booking_date, $start_time, $end_time, $requested_projectors, $requested_laptops, $user_id, $department, $phone_extension, $phone_number);
...
$sql = "SELECT COUNT(*) AS total_bookings FROM bookings WHERE customer_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();
...
// Add your HTML code here to display the booking form, the user's existing bookings, and the total number of bookings
?>
...
<div class="form-container">
    ...
    Booking Date: <input type="date" name="booking_date" required><br>
    Start Time: <input type="time" name="start_time" required><br> 
    End Time: <input type="time" name="end_time" required><br>
    Projectors: <input type="number" name="projectors" min="0" max="<?php echo $available_projectors; ?>" required><br> 
    Laptops: <input type="number" name="laptops" min="0" max="<?php echo $available_laptops; ?>" required><br>
    Department: <input type="text" name="department" required><br>
    CUT Phone Extension: <input type="text" name="phone_extension" required><br> 
